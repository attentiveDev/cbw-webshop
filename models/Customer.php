...
class Customer {
    ...
    public static function verifyUser($database, $username, $userpassword) {
        $stmt = $database->prepare('SELECT id, email, password, firstname, lastname FROM customers WHERE email=:email');
        $params = array('email' => trim(strtolower($username)));
        try {
            $stmt->execute($params);
        } catch (PDOException $error) {
            showFatalError("Database Error: " . $error->getMessage());
        ...
        if ($result) {
            ...
            if (password_verify($userpassword, $result["password"])) {
                # password correct
                $_SESSION["customerVerified"] = true;
                $_SESSION["customerId"] = $result["id"];
                $_SESSION["customerFullName"] = trim(trim($result["firstname"]) . " " . trim($result["lastname"]));
                return true;
            } else {
                # wrong password
                return false;
